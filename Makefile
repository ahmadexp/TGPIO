...
load: all
	$(SUDO) ADDR0="$(ADDR0)" ADDR1="$(ADDR1)" MMIO_SIZE="$(MMIO_SIZE)" USE_SECOND="$(USE_SECOND)" MODE0="$(MODE0)" MODE1="$(MODE1)" EDGE0="$(EDGE0)" EDGE1="$(EDGE1)" TIMESTAMP_MODE="$(TIMESTAMP_MODE)" OUTPUT_POLARITY="$(OUTPUT_POLARITY)" POLL_MS="$(POLL_MS)" ART_FREQUENCY="$(ART_FREQUENCY)" HARDWARE_TIMESTAMPS="$(HARDWARE_TIMESTAMPS)" ./scripts/load.sh
...
install: all
	$(SUDO) ADDR0="$(ADDR0)" ADDR1="$(ADDR1)" MMIO_SIZE="$(MMIO_SIZE)" USE_SECOND="$(USE_SECOND)" MODE0="$(MODE0)" MODE1="$(MODE1)" EDGE0="$(EDGE0)" EDGE1="$(EDGE1)" TIMESTAMP_MODE="$(TIMESTAMP_MODE)" OUTPUT_POLARITY="$(OUTPUT_POLARITY)" POLL_MS="$(POLL_MS)" ART_FREQUENCY="$(ART_FREQUENCY)" HARDWARE_TIMESTAMPS="$(HARDWARE_TIMESTAMPS)" ./scripts/install.sh
