...
\begin{abstract}
Clock synchronization makes timestamps comparable across systems, but measurement and control applications often need to bridge that time to physical signals. Time-Aware GPIO (TGPIO) provides this timed-I/O boundary, timestamping external edges and generating scheduled outputs in the platform time domain. This paper presents a Linux driver that exposes TGPIO blocks that the firmware does not enumerate through the standard PTP hardware clock interface, so unmodified tools such as \texttt{ts2phc} and \texttt{phc2sys} can drive otherwise unreachable hardware. Its core contribution is a set of output semantics found through register-level experiments (inert single-shot compares, an unreadable write-only level flop, and glitch-free in-flight register rewrites) and a discipline built on them that steers a free-running output without waveform restarts while tracking polarity in software. Measured against the atomic-clock-referenced PPS of an OCP TimeCard, the disciplined 1~PPS output holds a \ns{+9} mean offset (\ns{64} standard deviation), comparable to the TimeCard's own PPS.
\end{abstract}
...
Real time systems often need more than a synchronized network clock. They also often need physical I/O that binds external events to a shared timebase such as timestamping a pulse-per-second (PPS) signal, measuring an instrument trigger, or generating a scheduled output transition for a device under test. Linux already handles this class of ancillary work through its PTP hardware clock (PHC) abstraction, which covers both external timestamp input and periodic output generation \cite{linux-ptp-docs}.

Consider a system that must derive time from a GPS receiver without a network interface card PHC, FPGA, or dedicated timing card. TGPIO can timestamp the receiver's PPS edge directly and expose the capture as a PHC external timestamp event. Standard linuxptp tools can then discipline the TGPIO PHC and transfer the resulting timescale to the system clock. This provides a minimal-hardware path from an external reference into Linux timekeeping and allows the operating system to participate in time-sensitive applications, including Time-Sensitive Networking (TSN) \cite{ieee8021as}.

There is a challenge when employing certain commodity platforms: The timed I/O resources are physically present and functional, but firmware doesn't enumerate them, so they're not found by the normal platform-driver discovery path. The result is that applications do not have a standarized way to reach hardware useful for time synchronization and measurement.
...
The control surface is the standard PTP character device rather than a private ABI. \texttt{testptp} handles pin assignment, external timestamp enablement, and periodic-output requests, and PyPTM offers the same operations as a Python API. One compatibility detail is worth recording. Recent kernels check the strict \texttt{PTP\_EXTTS\_REQUEST2} ioctl's flags against the \texttt{supported\_extts\_flags} field of \texttt{ptp\_clock\_info} before the driver ever sees the request, so a driver that leaves that field zero quietly rejects every strict request even though the legacy ioctl still works. Advertising the supported flags fixed \texttt{ts2phc}, and it's worth checking in any PHC driver old enough to predate the field.
