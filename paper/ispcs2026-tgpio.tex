...
Consider a system that must derive time from a GPS receiver without a network interface card PHC, FPGA, or dedicated timing card. TGPIO can timestamp the receiver's PPS edge directly and expose the capture as a PHC external timestamp event. Standard linuxptp tools can then discipline the TGPIO PHC and transfer the resulting time to the system clock. This provides a minimal-hardware path from an external reference into Linux timekeeping and allows the operating system to participate in the timing domain of a Time-Sensitive Networking (TSN) system \cite{ieee8021as}.

There is a catch on some commodity platforms. The timed I/O resources are physically present and functional, but firmware never enumerates them, so the normal platform-driver discovery path finds nothing. The result is an awkward gap in that applications do not have a standarized way to reach hardware useful for synchronization and measurement.
...
Register access turns out to be the easy part. The harder problem is that TGPIO capture and compare values live in the platform ART domain, while the PTP clock interface speaks in a PTP clock domain. So the driver defaults to an ART-backed adjustable PHC, adds explicit realtime and raw ART modes for compatibility and diagnostics, and deals with the bring-up details that an architecture-level description tends to gloss over including input edge defaults, deterministic output startup, and output polarity.

This work makes the following contributions:
\begin{enumerate}
\item A PTP-clock abstraction for static TGPIO MMIO blocks that firmware does not enumerate, with per-block selection of external timestamp input, periodic output, or disabled operation.
\item A timestamp path that supports realtime-converted captures, raw ART-derived timestamps, and a default ART-backed adjustable PHC mode for tools such as \texttt{ts2phc}.
\item A set of experimentally established TGPIO output semantics %(toggle-only level state, inert single-shot compares, safe hot register rewrites) 
together with a discipline design built on them, combining grid-aligned re-arms, in-flight interval refresh, sub-\ns{100} phase nudges, and software-tracked polarity.
\item A one-shot external calibration that cancels constant clock-comparison and pipeline delays, holding a disciplined 1~PPS output within \ns{100} of an atomic reference.
\item A Windows KMDF port of the same blocks that reuses those hardware findings unchanged, which separates what is portable about timed I/O from what each operating system's clock model forces.
\end{enumerate}
...
Figure~\ref{fig:architecture} shows the overall architecture.  The driver maps the static TGPIO blocks, presents each as a PTP pin, and converts between the hardware ART domain and the PTP clock domain on both the input and output paths.
...
Second, the modes need different dependency checks: raw ART conversion and hardware periodic intervals require an ART frequency value (supplied or CPUID-detected), while realtime conversion relies on Linux timekeeping's ART clock relationship; the driver reports the CPUID values for diagnostics either way.

Third, the output semantics above differed substantially from the initial mental model and were settled only by direct register experiments observed with a logic analyzer.  An initially implemented ``preconditioning edge'' relied on single-shot comparison and was a silent no-op, the edge-polarity field has no effect on outputs, and no register round-trip reveals the output level. The belief that in-flight register rewrites corrupt the waveform originated from an older driver revision and dissolved under controlled experiments. The one caveat is that a comparator rewrite can register a phantom event that flips the level-mirror parity, so the driver pauses the periodic-mode bit around such rewrites while output enable holds the line level. Because the analyzer captures were scripted, each wrong hypothesis was falsifiable within minutes; this loop of poke, capture, and decode was the single most productive bring-up tool.
